...
export const ServiceBusinessInputForm: React.FC<ServiceBusinessInputFormProps> = ({ onSubmit, onBack, initialValues }) => {
  ...
  const addLoan = () => setLoans(l => [...l, { amount: '', rate: '', years: '', startDate: '' }]);
          ...
          {hasLoan && (
            <div className="space-y-2">
              ...
              <div className="flex gap-x-4 items-center mb-2 px-1">
                ...
                <div className="text-xs font-semibold text-muted-foreground w-20">Rate %</div>
                <div className="text-xs font-semibold text-muted-foreground w-20">Years</div>
                <div className="text-xs w-10" />
              </div>
              {loans.map((loan, idx) => (
                <div key={idx} className="flex gap-x-4 items-center mb-2 px-1">
                  <Input placeholder="$10000" value={loan.amount} onChange={e => handleLoanChange(idx, 'amount', e.target.value)} className="w-32" />
                  <Input placeholder="5%" value={loan.rate} onChange={e => handleLoanChange(idx, 'rate', e.target.value)} className="w-20" />
                  <Input placeholder="3" value={loan.years} onChange={e => handleLoanChange(idx, 'years', e.target.value)} className="w-20" />
